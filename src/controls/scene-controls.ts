...
// type backGround = "White" | "Black" | "Null" | "Color" | "Texture" | "Cubemap";
...
const fogProperties = (fog: THREE.Fog) => ({
  color: 0xffffff,
  near: fog.near,
  far: fog.far,
});

export const initializeSceneControls = (
  gui: GUI,
  scene: THREE.Scene,
  fogEnabled?: boolean,
  isOpen?: boolean
  ...
  sceneControls
    ...
    .onChange((event: string) => handleBackgroundChange(event, scene));
...
const handleBackgroundChange = (setting: string, scene: THREE.Scene) => {
  switch (setting) {
    case "White":
